...
def make_sample_her_transitions(replay_strategy, replay_k, reward_fun, obs_to_goal_fun=None, no_her=False):
    ...
    def _sample_her_transitions(episode_batch, batch_size_in_transitions, info=None):
        ...
        if info.get('et', False):
            transitions['r'] = reward + gamma *  Q_pi_fun(o=transitions['o_2'], g=transitions['g']).reshape(-1) * (1-success)
        else:
            transitions['r'] = reward + gamma *  Q_pi_fun(o=transitions['o_2'], g=transitions['g']).reshape(-1)
    ...
    def _sample_nstep_her_transitions(episode_batch, batch_size_in_transitions, info):
        steps, gamma, Q_fun, truncate = info['nstep'], info['gamma'], info['get_Q_pi'], info['truncate']
        transitions, episode_idxs, t_samples, batch_size, T = _preprocess(episode_batch, batch_size_in_transitions)
        ...
        transitions['r'] = (n_step_reward * n_step_reward_mask * terminal_mask).sum(axis=1).copy()
        ...
        if info.get('et', False):
            success_within_n_steps = np.max(n_step_success, axis=1)
            transitions['r'] += (n_step_gamma * Q_fun(o=n_step_os, g=transitions['g'])).reshape(-1) * (1-success_within_n_steps)
        else:
            transitions['r'] += (n_step_gamma * Q_fun(o=n_step_os, g=transitions['g'])).reshape(-1)
        if np.random.random() < 0.001:
            print(np.mean(transitions['r']))
        return _reshape_transitions(transitions, batch_size, batch_size_in_transitions)
    ...
    def _sample_nstep_lambda_her_transitions(episode_batch, batch_size_in_transitions, info):
        ...
        for i in range(steps):
            ...
            if info.get('et', False):
                ...
                return_i = (n_step_reward[:,:i+1] * n_step_reward_mask[:,:i+1] * terminal_mask[:, :i+1]).sum(axis=1) + i_step_gamma * Q_pi_fun(
                        o=i_step_o2s,
                        g=transitions['g']).reshape(-1) * (1 - success_within_i_steps)
            else:
                return_i = (n_step_reward[:,:i+1] * n_step_reward_mask[:,:i+1] * terminal_mask[:, :i+1]).sum(axis=1) + i_step_gamma * Q_pi_fun(
                        o=i_step_o2s,
    ...
    def _sample_nstep_dynamic_her_transitions(episode_batch, batch_size_in_transitions, info):
        ...
        if steps > 1:
            for _ in range(1, steps):
                ...
                action_array, Q_pi = action_and_Q_pi_fun(o=state_array, g=transitions['g'])
                ...
                Q_pi_cache.append(Q_pi.reshape(-1))
        ...
        Q_pi_cache.append(Q_pi_fun(o=last_state, g=transitions['g']).reshape(-1))
        ...
        if info.get('et', False):
            transitions['r'] += pow(gamma, truncated_n_steps) * Q_pi_cache[np.arange(batch_size), truncated_n_steps-1].reshape(-1) * (1-success_within_n_steps)
            target_step1 = reward_list[0] + gamma * Q_pi_cache[:, 0] * (1-n_step_success[:, 0])
        else:
            transitions['r'] += pow(gamma, truncated_n_steps) * Q_pi_cache[np.arange(batch_size), truncated_n_steps-1].reshape(-1)
