...
class RolloutWorker:
    ...
    def __init__(self, venv, policy, dims, logger, T, rollout_batch_size=1,
                 ...
                 random_eps=0, history_len=100, render=False, monitor=False, debug_dir=None, **kwargs):
    ...
    def generate_rollouts(self, random_ac=False):
        ...
        if self.compute_Q:
            ...
            self.return_history.append(np.mean(np.sum(np.array(rewards).T, axis=1)))
        self.n_episodes += self.rollout_batch_size

        if self.exploit:
            QA = np.stack(Qs, axis=1)
    ...
    def logs(self, prefix='worker'):
        ...
        if self.compute_Q:
            ...
            logs += [('mean_return', np.mean(self.return_history))]
