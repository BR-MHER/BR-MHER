...
def expectile_huber_loss(target, v_to_goal, tau, delta=1.0):
    ...
    linear_loss = delta * (2 * abs_diff - delta)
...
class DDPG(object):
    ...
    def __init__(self, action_space, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
                 ...
                 reuse=False, grad_clip_value=-1, et=False, scale_degree=1.0, truncate=False, 
                 tau=0.7, delta=1.0, use_huber=False, normalize_q_pi=False, policy_delay=1,
                 noise_std=0, noise_clip=0.5, **kwargs):
        ...
        assert 0.5<= self.tau <= 1.0, "the tau should be between 0.5 and 1.0"
        ...
        # stage_shapes['r'] = (None,)
        stage_shapes['target_q'] = (None,)
        # stage_shapes['r'] = (None,)
        # stage_shapes['reward'] = (None,)
        stage_shapes['success_within_n_steps'] = (None,)
        stage_shapes['success_within_one_step'] = (None,)
        stage_shapes['truncated_n_steps'] = (None,)
    ...
    def set_epoch(self, epoch, save_path):
        self._epoch = epoch
        self._save_csv_path = os.path.join(logger.get_dir(), "epoch_{}_data.csv".format(epoch))

    def save_last_epoch_samples(self, target_diff, critic_values, num_samples=50):
        if self._epoch < 49:
            return

        num_values = critic_values.shape[1]
        data_to_save = {f"critic_value_{i}": critic_values[:num_samples, i] for i in range(num_values)}
        data_to_save.update({f"target_diff_{i}": target_diff[:num_samples, i] for i in range(num_values)})

        # Create a DataFrame from the sampled data
        df = pd.DataFrame(data_to_save)
    
        # Append the DataFrame to the CSV file
        if os.path.isfile(self._save_csv_path):
            # If file exists, append without writing headers
            df.to_csv(self._save_csv_path, mode='a', header=False, index=False)
        else:
            # If file does not exist, write DataFrame to CSV file with headers
            df.to_csv(self._save_csv_path, mode='w', header=True, index=False)
    ...
    # NOTE this seemed to be only useful for model-based mothod, will fix it later.
    def get_action_and_Q_pi(self, o, g, success):
        o = np.clip(o, -self.clip_obs, self.clip_obs)
        g = np.clip(g, -self.clip_obs, self.clip_obs)
        ...
        action, q_pi = self.sess.run([policy.pi_tf, policy.Q_pi_tf], feed_dict={
            policy.o_tf: o.reshape(-1, self.dimo),
            policy.g_tf: g.reshape(-1, self.dimg),
            policy.success_tf: success.reshape(-1, 1)
        })
        return action, q_pi

    def get_actions(self, o, ag, g, success, noise_eps=0., random_eps=0., use_target_net=False,
                    compute_Q=False):
        ...
        if compute_Q: # compute the IQR Q at the same time.
            vals += [policy.Q_pi_tf]
        # feed
        feed = {
            policy.o_tf: o.reshape(-1, self.dimo),
    ...
    def _grads(self):
        ...
        loss_scale, success_1_proportion, success_n_proportion, critic_loss, actor_loss, \
            Q_grad, pi_grad, Q_grad_norm, pi_grad_norm, target_diff, \
                q_to_goal = self.sess.run([
        ...
        self.save_last_epoch_samples(target_diff, q_to_goal)
        ...
        self.debug_stats["loss_scale"].append(loss_scale)
    ...
    def _update(self, Q_grad, pi_grad):
        ...
        if self._update_t % self.policy_delay == 0:
    ...
    def sample_batch(self, method='list'):
        ...
        transitions['o'], transitions['g'] = self._preprocess_og(o, ag, g)
        transitions['o_2'], transitions['g_2'] = self._preprocess_og(o, ag, g)
        # transitions['o_2'], transitions['g_2'] = self._preprocess_og(o_2, ag_2, g)

        self.debug_stats['truncated_n_steps'] = np.mean(transitions['truncated_n_steps'])
    ...
    def _create_network(self, reuse=False):
        ...
        batch_tf['target_q'] = tf.reshape(batch_tf['target_q'], [-1, 1])
        batch_tf['success_within_n_steps'] = tf.reshape(batch_tf['success_within_n_steps'], [-1, 1])
        batch_tf['success_within_one_step'] = tf.reshape(batch_tf['success_within_one_step'], [-1, 1])
        batch_tf['truncated_n_steps'] = tf.reshape(batch_tf['truncated_n_steps'], [-1, 1])
        ...
        with tf.variable_scope('main') as vs:
            ...
            main_dict["noise_std"] = 0
            self.main = self.create_actor_critic(batch_tf, net_type='main', **main_dict)
            vs.reuse_variables()
        ...
        # Take the target network of the first value network.
        target_q_to_goal = tf.stop_gradient(self.target.Q1_tf)

        clip_range = (-self.clip_return, 0. if self.clip_pos_returns else np.inf)
        self.target_q_tf = tf.clip_by_value(batch_tf['target_q'] , *clip_range)  # lambda target 

        targe_q_tf = tf.stop_gradient(self.target_q_tf)

        if self.use_huber:
            Q1_loss = expectile_huber_loss(targe_q_tf, q1_to_goal, self.tau, self.delta)

            Q2_loss = expectile_huber_loss(targe_q_tf, q2_to_goal, self.tau, self.delta)
        else:
            Q1_loss = expectile_loss(normalized_targe_q_tf, q1_to_goal, self.tau)

            Q2_loss = expectile_loss(normalized_targe_q_tf, q2_to_goal, self.tau)

        loss_scale = tf.pow(self.sqrt_n_step/batch_tf['truncated_n_steps'], self.scale_degree)
        self.mean_loss_scale = tf.reduce_mean(loss_scale)

        
        self.target_diff = tf.stop_gradient(self.target_q_tf) - q1_to_goal
        self.q_to_goal = q1_to_goal
        self.Q_loss_tf = tf.reduce_mean((Q1_loss + Q2_loss) * loss_scale)
        ...
        # NOTE This might help the learning more stable without the influence of the value scales.
        if self.normalize_q_pi:
            normalized_q_pi_to_goal = q_pi_to_goal / sqrt_iqr_clamp
            self.pi_loss_tf = -tf.reduce_mean(normalized_q_pi_to_goal[:, 0, None]) # only use the largest quantile.
        else:
            self.pi_loss_tf = -tf.reduce_mean(q_pi_to_goal[:, 0, None]) # only use the largest quantile.
